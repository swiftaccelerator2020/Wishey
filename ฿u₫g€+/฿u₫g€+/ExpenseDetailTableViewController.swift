...
//  ฿u₫g€+
//
//  Created by Granwyn Tan on 16/12/20.
//

import UIKit

class ExpenseDetailTableViewController: UITableViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        categoryName.text = expense.categoryName
        budgetAmount.text = String(expense.projectedExpenses)
        categoryAmount.text = String(format: "%.2f", expense.actualExpenses)
    ...
    @IBAction func save(_ sender: Any) {
        if categoryName.text != nil && !categoryName.text!.isEmpty && categoryAmount.text != nil && !categoryAmount.text!.isEmpty && budgetAmount.text != nil && !budgetAmount.text!.isEmpty {
            if Double(categoryAmount.text!) != nil && Int(budgetAmount.text!) != nil {
                ...
                performSegue(withIdentifier: "unwindHomeExpense", sender: nil)
            } else {
